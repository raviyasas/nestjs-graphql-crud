...
export class ClientRepository extends BaseRepository<Client> implements ClientRepositoryInterface {
    ...
    async createClient(client: Client): Promise<Client> {
        try {
            ...
            console.error(`Failed to create a new client ${error.message}`);
            throw error;
        }
    }
    ...
    async getClient(clientId: string): Promise<Client> {
        try {
            ...
            console.error(`Failed to retrieve a client: ${error.message}`);
            throw error;
        }
    }
    ...
    async deleteClient(clientId: string): Promise<void> {
        ...
        try {
            const client = await this.clientRepository.findOneB({
                where: {
                    id: clientId
                }
            });
